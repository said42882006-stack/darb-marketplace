...
export default function PostAdFlow({
  ...
  const hasQuota = hasActiveSubscription || remainingFree > 0;
  ...
  const publish = async () => {
    ...
    if (!res.ok || !data.success) {
      if (data.code === "NEEDS_SUBSCRIPTION") setRemainingFree(0);
      ...
      {hasActiveSubscription ? (
        ...
        <div className="mb-6 rounded-xl border border-amber/40 bg-sand p-4 flex flex-col gap-3">
          <p className="text-sm text-ink">
            استخدمت إعلاناتك المجانية الثلاثة. اشترك بباقة لتنشر بلا حدود.
          </p>
          <a
            href="/pricing"
            className="rounded-xl py-2.5 font-bold bg-teal text-white text-center hover:bg-teal-deep transition-colors"
          >
            عرض باقات الاشتراك
          </a>
        </div>
      ...
      {step === "details" && (
        <div className={`flex flex-col gap-3 ${!hasQuota ? "opacity-50 pointer-events-none" : ""}`}>
          ...
          <div className="flex gap-3">
            ...
            <label className="text-sm font-medium text-ink flex-1">
              رقم التواصل (واتساب) *
              <input required value={form.ownerPhone} onChange={(e) => setForm({ ...form, ownerPhone: e.target.value })}
                className="mt-1 w-full rounded-lg border border-line px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal"
